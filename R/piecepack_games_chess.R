...
piecepack_baroque_chess <- function(has_subpack = FALSE) {
	...
	df_p2 <- piecepack_coins(side = "back", suit = 1:8 %% 2 + 3, rank = NA_integer_, x = 1:8, y = 2)
...
piecepack_chinese_chess <- function(has_subpack = FALSE) {
	...
	df_t2 <- piecepack_tiles(suit = c(1, 3), rank = 2L, x = 5, y = c(9, 2), angle = c(180, 0))
	df_zu1 <- piecepack_coins(
		side = "back",
		suit = (1:5 + 1) %% 2 + 3,
		rank = c(1, 4, 6, 1, 4),
		x = seq(1, 9, 2),
...
piecepack_four_seasons_chess <- function(has_subpack = FALSE) {
	...
	df_k <- piecepack_pawns(suit = suits, angle = angles, x = c(1, 8, 8, 1), y = c(8, 8, 1, 1))
	df_r <- piecepack_dice(
		suit = suits,
		rank = 4L,
		x = c(2, 7, 7, 2),
...
piecepack_chess_pieces <- function(
	...
	if (has_subpack) {
		...
		df_r <- piecepack_dice(rank = 4L, x = xr(4L), y = c(8, 8, 1, 1), angle = c(180, 180, 0, 0))
		df_n <- piecepack_coins(
			side = "face",
			rank = 2,
			suit = 1:4,
