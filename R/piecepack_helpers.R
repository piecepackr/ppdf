...
generate_helper <- function(suit, rank, angle = 0, which_ = "sra", ...) {
	...
	r <- rank - 1
	a <- "^"
	if (near(angle %% 360, 90)) {
		a <- "<"
	}
	if (near(angle %% 360, 180)) {
		a <- "v"
	}
	if (near(angle %% 360, 270)) {
		a <- ">"
	}
	switch(which_, sra = paste0(s, r, a), sr = paste(s, r), r = r)
}

process_tiles <- function(tiles = NULL, n_tiles = 24) {
	if (is.null(tiles)) {
		return(tibble(suit = rep(1:4, each = 6L), rank = rep.int(1:6, 4L), angle = 0)[
			sample.int(24L),
		])
	}
	...
	tibble(suit = as.integer(suits), rank = as.integer(ranks), angle = as.double(angles))
}
...
piecepack_dice <- function(
	...
	check_dots_empty()
	tibble(
		piece_side = rep("die_face", length.out = length.out),
		suit = rep(piece_suit(suit), length.out = length.out),
		rank = rep(piece_rank(rank), length.out = length.out),
		cfg = rep(cfg, length.out = length.out),
		x = rep(as.double(x), length.out = length.out),
