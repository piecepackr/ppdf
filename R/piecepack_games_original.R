...
piecepack_games_original <- function() {
	tribble(~game
            ...
            , "``piecepack_dominoids()``"
            , NA_character_
            , "https://ludism.org/ppwiki/Dominoids"
            , "Easy Slider"
            ...
            , "``piecepack_ship_it()``"
            , NA_character_
            , "https://ludism.org/ppwiki/Ship_It"
            , "Shopping Mall"
...
piecepack_black_pawn_trucking <- function(seed = NULL) {
	...
	df_ <- filter(df_tiles, paste(.data$rank, .data$suit) %in% paste(df_dice$rank, df_dice$suit))
	df_pawn <- filter(df_, .data$suit == 2L) |>
		mutate(piece_side = "pawn_face", rank = 1L, x = .data$x + 0.5, y = .data$y + 0.5)
	df_coins <- mutate(df_, piece_side = "coin_back", x = .data$x + 0.5, y = .data$y - 0.5)
...
piecepack_burbuja <- function(seed = NULL, tiles = NULL) {
	...
	df_coins <- piecepack_coins(
		side = rep(c(rep_len("back", 5), "face"), 4),
		rank = c(
			(1:6)[sample.int(6)],
			(1:6)[sample.int(6)],
			(1:6)[sample.int(6)],
...
piecepack_cardinals_guards <- function(seed = NULL, tiles = NULL) {
	...
	df_dice <- piecepack_dice(rank = 1, x = 16, y = seq.int(from = 2, by = 2, length.out = 4))
...
piecepack_chariots <- function() {
	piecepack_tiles(
		side = "back",
		x = c(rep(seq(5.5, 9.5, 2), 4), rep(c(3.5, 11.5), 4), rep(c(1.5, 13.5), 2)),
		y = c(
			rep(c(9.5, 7.5, 3.5, 1.5), each = 3),
...
piecepack_crocodile_hop <- function(seed = NULL) {
	...
	repeat {
		# re-roll die if equals bottom-right tile (removed lily pond)
		df_dice <- piecepack_dice(x = seq(6, length.out = 4), y = 5, rank = random_dice())
	...
	df_pawns <- filter(
		df_coins,
		paste(.data$suit, .data$rank) %in% paste(df_dice$suit, df_dice$rank)
	) |>
...
piecepack_dominoids <- function(..., seed = NULL) {
	...
	df_board <- piecepack_rectangular_board(
		8L,
		8L,
		suit = c(rep.int(c(1:2, 3L), c(6L, 6L, 4L))),
		rank = c(1:6, 1:6, 1:4)
	)
	df_pawns <- piecepack_pawns(x = c(2, 3, 7, 6), y = c(3, 7, 6, 2), suit = sample.int(4L))
	df_coins <- piecepack_coins(
		side = "back",
		rank = as.integer(replicate(4L, sample.int(6L))),
		x = rep(c(2, 4, 6, 8), each = 6L),
		y = 10L
	)
	s1 <- df_pawns$suit[1L]
	i1 <- 6L * (s1 - 1L) + 1L
...
piecepack_everest <- function() {
	...
	df_t2 <- piecepack_rectangular_board(
		4,
		6,
		x0 = 2,
		y0 = 2,
	...
	df_t3 <- piecepack_rectangular_board(
		4,
		4,
		x0 = 3,
		y0 = 2,
	...
	df_t4 <- piecepack_rectangular_board(
		2,
		4,
		x0 = 3,
		y0 = 3,
...
piecepack_galaxy_express <- function(seed = NULL) {
	...
	df_speed_tiles <- piecepack_tiles(x = 10.5, y = c(1.5, 5.5), rank = 2L, suit = c(2L, 1L))
	df_speed_coins <- piecepack_coins(
		...
		x = 12,
		y = rep.int(c(7, 6, 5, 3, 2, 1), c(4, 1, 1, 4, 1, 1))
	)
	bind_rows(df_board, df_arms, df_crowns, df_dice, df_speed_tiles, df_speed_coins, df_pawn)
}
...
piecepack_japan <- function(seed = NULL) {
	...
	df_tiles <- piecepack_tiles(
		...
		x = 0.5 +
			c(
				rep(seq(1, by = 4, length.out = 4), each = 3),
				rep(seq(3, by = 4, length.out = 3), each = 4)
			),
		y = 0.5 + c(rep(seq(2, by = 2, length.out = 3), 4), rep(seq(1, by = 2, length.out = 4), 3))
	)
...
piecepack_pawns_crossing <- function(..., seed = NULL, n_players = 2L) {
	...
	if (n_players == 2L) {
		df_p <- piecepack_pawns(x = c(4, 5, 8, 7), y = c(13, 13, -1, -1), angle = c(180, 180, 0, 0))
		df_d <- piecepack_dice(
			x = c(1, 2, 11, 10),
			y = c(13, 13, -1, -1),
			rank = random_dice(),
...
piecepack_roundabout <- function(seed = NULL) {
	df_c <- piecepack_coins(
		...
		suit = c(4, 3, 2, 1),
		rank = c(
			sample(setdiff(2:6, 2L), 1L), # not ace of arms
			sample(setdiff(2:6, 4L), 1L), # not 3 of crowns
			sample(setdiff(2:6, 5L), 1L), # not 4 of moons
			sample(setdiff(2:6, 3L), 1L)
		)
	) # not 2 of suns
	df_p <- piecepack_pawns(x = c(5, 5, 6, 6), y = c(5, 6, 6, 5), suit = c(4, 2, 3, 1))
	df_n <- piecepack_coins(x = 6, y = 4, rank = 1L, suit = 1L)
	df_mav <- piecepack_matchsticks(
		rank = 2L,
		x = 0.5 + c(4, 4, 6, 6, 5, 5),
		y = c(6, 8, 8, 5, 2:3),
...
piecepack_one_man_thrag <- function(seed = NULL) {
	...
	df_coins <- piecepack_coins(
		...
		y = 5,
		suit = rep.int(c(1L, 3L, 4L, 2L), c(6L, 6L, 6L, 3L)),
		rank = c(sample.int(6), sample.int(6), sample.int(6), c(2, 4, 6)[sample.int(3)])
	)
	df_pawns <- piecepack_pawns(x = c(1, 6, 8, 10), y = c(4, 6, 6, 6), suit = c(2L, 1L, 3L, 4L))
	df_health <- piecepack_coins(side = "face", x = 13, y = 6:8, suit = 2L, rank = c(1L, 3L, 5L))
...
new_impossible <- function(x_y) {
	...
	c(p_(xy[1] - 1L, xy[2] + -1:1), p_(xy[1], xy[2] + -1:1), p_(xy[1] + 1L, xy[2] + -1:1))
}
...
piecepack_ship_it <- function(..., seed = NULL) {
	...
	df_coins <- piecepack_coins(
		side = "back",
		suit = rep(c(1L, 3L, 4L, 2L), each = 5L), # Order RGBK
		rank = as.integer(replicate(4L, sample(2:6))),
		x = NA_integer_,
		y = NA_integer_
	) |>
		slice(c(1, 6, 11, 16, 2, 7, 12, 17, 3, 8, 13, 18, 4, 9, 14, 19, 5, 10, 15, 20))
	df_pawns <- piecepack_pawns(x = NA_integer_, y = NA_integer_)
	df_dice <- piecepack_dice(x = NA_integer_, y = NA_integer_, rank = 2L)
	city_row <- 1L # Tile column within a city (top to bottom)
	city_col <- 1L # Tile column within a city (left to right)
	city_suits <- integer(0L) # Suits in the city
	city_length <- 0L # Number of tiles in the city
	tile_y <- 13.5
	tile_x <- 1.5
	col_length <- 1L # Number of cities in column
	i_coin <- 1L
	for (i in seq.int(24L)) {
		df_tiles$y[i] <- tile_y
		df_tiles$x[i] <- tile_x
		city_length <- city_length + 1L
		city_suits <- c(city_suits, df_tiles$suit[i])
		if (df_tiles$angle[i] == 0L) {
			xo <- 0.5
			yo <- -0.5
		} else if (df_tiles$angle[i] == 90L) {
			xo <- 0.5
			yo <- 0.5
		} else if (df_tiles$angle[i] == 180L) {
			xo <- -0.5
			yo <- 0.5
		} else {
			xo <- -0.5
		...
		if (df_tiles$rank[i] == 1L) {
			# Put die of same suit on null tiles
			df_dice$y[df_tiles$suit[i]] <- tile_y + yo
			df_dice$x[df_tiles$suit[i]] <- tile_x - xo
		}
		if (df_tiles$rank[i] != 2L) {
			# Don't put coins on ace tiles
			df_coins$y[i_coin] <- tile_y + yo
			df_coins$x[i_coin] <- tile_x + xo
			i_coin <- i_coin + 1L
		} else {
			# Put pawn of same suit on ace tiles
			df_pawns$y[df_tiles$suit[i]] <- tile_y + yo
			df_pawns$x[df_tiles$suit[i]] <- tile_x + xo
		}
		if (i == 24L) {
			# No more tiles to place
			invisible(NULL)
		} else if (df_tiles$suit[i + 1L] %in% city_suits || city_length == 3L) {
			# New city
			city_row <- 1L
			city_length <- 0L
			city_suits <- integer(0L)
			if (col_length == 3L) {
				# Start a new column
				col_length <- 1L
				city_col <- 1L
				tile_y <- 13.5
				tile_x <- max(df_tiles$x, na.rm = TRUE) + 4
			} else {
				# Down a row in same column
				col_length <- col_length + 1L
				tile_y <- tile_y - 4
			}
		} else {
			# Add to an existing city
			if (city_row == 1L && city_col == 1L) {
				city_col <- 2L
				tile_x <- tile_x + 2
			} else if (city_row == 1L && city_col == 2L) {
				city_row <- 2L
				tile_y <- tile_y - 2
			} else if (city_row == 2L && city_col == 2L) {
				city_col <- 1L
				tile_x <- tile_x - 2
			}
		}
	}
	df_dominoes <- domino_tiles(
		side = "back",
...
ship_it_city_x <- function(row, col, city_row, city_col) {
	6 * (col - 1) + 2 * city_col - 1 + 0.5
}
...
piecepack_skyscrapers <- function(seed = NULL, tiles = NULL) {
	...
	df_pawn <- mutate(df_pawn, piece_side = "pawn_face", x = .data$x + 0.5, y = .data$y - 0.5)
	bind_rows(df_tiles, df_pawn)
}
...
piecepack_speedy_towers <- function(n_players = 2, seed = NULL) {
	...
	if (n_players == 2) {
		...
		df_coins <- piecepack_coins(
			...
			slice_sample_piece() |>
			slice(c(order(.data$suit[1:12]), 12 + order(.data$suit[13:24]))) |>
			mutate(
				x = c(seq(2, by = 2, length.out = 12), seq(24, by = -2, length.out = 12)),
				y = c(rep(2, 12), rep(14, 12)),
				angle = c(rep_len(0, 12), rep_len(180, 12))
			)
		df_pawns <- piecepack_pawns(suit = c(3, 1), x = c(24, 2), y = c(4, 12), angle = c(0, 180))
		bind_rows(df_tiles, df_coins, df_pawns)
	} else {
...
piecepack_steppin_stones <- function(seed = NULL) {
	...
	df_tiles_backs <- piecepack_tiles(
		side = "back",
		suit = rep(1:4, each = 3),
		rank = rep(3:5, 4),
		x = c(3.5, 1.5, 3.5, 5.5, 5.5, 7.5, 5.5, 5.5, 7.5, 3.5, 3.5, 1.5),
		y = c(7.5, 5.5, 5.5, 7.5, 5.5, 5.5, 3.5, 1.5, 3.5, 3.5, 1.5, 3.5)
	)
	df_coins <- piecepack_coins(
		suit = rep(1:4, each = 5),
		rank = rep(c(1, 3:6), 4),
		x = c(3, 2, 1, 3, 3, 6, 7, 8, 6, 6, 6, 7, 8, 6, 6, 3, 2, 1, 3, 3),
		y = c(6, 6, 6, 7, 8, 6, 6, 6, 7, 8, 3, 3, 3, 2, 1, 3, 3, 3, 2, 1),
...
piecepack_tower_of_babel <- function(seed = NULL, tiles = NULL) {
	...
	df <- mutate(df, piece_side = "tile_back", x = 2, y = 4, cfg = "piecepack", angle = 0) |>
		select_piece()
	df[24L, "y"] <- 2
...
piecepack_tracers <- function() {
	...
	df_pawns <- piecepack_pawns(suit = 1:2, x = c(1, 8), y = c(1, 8), angle = c(0, 180))
...
piecepack_triactor <- function(seed = NULL, cfg2 = "playing_cards_expansion") {
	...
	df_c1 <- piecepack_coins(
		side = "back",
		suit = 1:4,
		rank = sample.int(6L, 4L, replace = TRUE),
		x = 0.5 + c(5, 15, 15, 5),
		y = 0.5 + c(11, 11, 1, 1)
	)
	df_c2 <- piecepack_coins(
		side = "back",
		cfg = cfg2,
		suit = 1:4,
		rank = sample.int(6L, 4L, replace = TRUE),
	...
	df_p <- piecepack_pawns(
		cfg = rep(c("piecepack", cfg2), each = 4),
		x = 10.5,
		y = 0.5 + 0:7,
		angle = 90,
		suit = rep.int(1:4, 2L)
...
piecepack_tula <- function(
	...
	df_txy <- switch(
		variant,
		Original = tibble(
			piece_side = c(rep("tile_back", 23), "tile_face"),
			x = c(rep(seq(1, 7, 2), 3), 4, rep(seq(2, 6, 2), 2), 4, 3, 5, 4, 4),
			y = c(rep(seq(7, 3, -2), each = 4), 1, rep(c(6, 4), each = 3), 2, 5, 5, 3, 4)
		),
		`Variant 1` = tibble(
			piece_side = c(rep("tile_back", 21), "tile_face", "tile_back", "tile_face"),
			x = c(seq(2, 8, 2), seq(1, 9, 2), seq(2, 8, 2), rep(seq(2, 8, 2), 2), seq(3, 7, 2)),
			y = c(rep.int(seq(5, 1, -2), c(4, 5, 4)), rep(c(4, 2), each = 4), rep(3, 3))
		),
		`Variant 2` = tibble(
			piece_side = c(
				rep("tile_back", 28),
			...
			y = c(
				rep.int(seq(11, 1, -2), c(2, 5, 7, 7, 5, 2)),
		...
		`Variant 4` = tibble(
			...
			y = c(
				rep.int(c(7, 5, 4, 3, 1), c(5, 6, 2, 6, 5)),
