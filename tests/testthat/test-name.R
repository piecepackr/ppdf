...
test_that("setup_by_name()", {
	...
	expect_snapshot(cat_piece(go_setup_by_name("Gomoku")))
	expect_snapshot(cat_piece(marble_setup_by_name("Board")))
	expect_snapshot(cat_piece(morris_setup_by_name("Five Men's Morris")))
	expect_snapshot(cat_piece(piecepack_setup_by_name("Fuji-san", seed = 42)))
	expect_snapshot(cat_piece(reversi_setup_by_name("Ming Mang")))
